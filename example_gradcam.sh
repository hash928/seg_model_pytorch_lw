...
# 示例 3: 使用 ROI 掩码 最佳参数，后面不要改动这个示例
python visualize_gradcam.py \
  --image_path "/home/data/sam-unet/shi_ce/xiangdao_data12/Test_Images/jz_label_67.png" \
  --checkpoint "checkpoints/unet_resnet34_aspp_se_dice_bce_loss_training_Mosaic_4_seed512/unet_resnet34-best.pth" \
  --model_type "unet_resnet34" \
  --num_classes 1 \
  --use_aspp \
  --attention_type "se" \
