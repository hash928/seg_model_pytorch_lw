...
def parse_shell_args(shell_file='test_sam2.sh'):
    """从shell脚本中解析参数，适配 test_sam2.sh 的参数格式"""
    args_dict = {}
    try:
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser("SAM2 Model Testing")
...
def test_single_image(predictor, image, mask, input_point, device, save_path=None, sample_idx=0, original_mask_path=None, mask_name=None):
    ...
    try:
        ...
        # 可视化结果
        if save_path:
            visualize_result(image, mask, prd_mask.detach().cpu().numpy(), input_point, 
                           iou, dice, save_path, sample_idx, original_mask_path, mask_name)
        
        return {
            'iou': iou,
            'dice': dice,
            'prediction': prd_mask.detach().cpu().numpy(),
            'gt_mask': mask,
...
def visualize_result(image, gt_mask, pred_mask, input_point, iou, dice, save_path, sample_idx, original_mask_path=None, mask_name=None):
    ...
    if len(image.shape) == 3 and image.shape[0] == 1:
        # 如果是 (1, H, W) 格式，转换为 (H, W)
        image = image.squeeze(0)
    elif len(image.shape) == 3 and image.shape[2] == 1:
        # 如果是 (H, W, 1) 格式，转换为 (H, W)
    ...
    if len(pred_mask.shape) == 3 and pred_mask.shape[0] == 1:
        ...
        print(f"预测掩码尺寸: {pred_mask.shape}")
...
def main(args):
    ...
    try:
        ...
        # 使用相对路径加载配置文件
        config_name = os.path.basename(args.model_cfg)
        print(f"使用配置文件: {config_name}")
        
        # 首先构建基础模型
        sam2_model = build_sam2(config_name, device=device)
