...
class SegNet(nn.Module):
    ...
    def __init__(self, n_channels: int = 3, n_classes: int = 1, BN_momentum: float = 0.5):
        ...
        # 将通道整体缩小 2 倍：64,128,256,512,512 -> 32,64,128,256,256
        self.ConvEn11 = nn.Conv2d(self.in_chn, 32, kernel_size=3, padding=1)
        self.BNEn11 = nn.BatchNorm2d(32, momentum=BN_momentum)
        self.ConvEn12 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.BNEn12 = nn.BatchNorm2d(32, momentum=BN_momentum)

        self.ConvEn21 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.BNEn21 = nn.BatchNorm2d(64, momentum=BN_momentum)
        self.ConvEn22 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.BNEn22 = nn.BatchNorm2d(64, momentum=BN_momentum)
        ...
        # 解码部分通道也按 2 倍缩小，并与对应 encoder 池化前通道对齐：
        # encoder 池化前通道：32, 64, 128, 256, 256
        ...
        self.BNDe31 = nn.BatchNorm2d(64, momentum=BN_momentum)
        ...
        self.BNDe21 = nn.BatchNorm2d(32, momentum=BN_momentum)

        self.ConvDe12 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.BNDe12 = nn.BatchNorm2d(32, momentum=BN_momentum)
        self.ConvDe11 = nn.Conv2d(32, self.out_chn, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # 返回 logits，交由损失函数内部做 sigmoid/softmax
        return x